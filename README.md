...
1. The footprint resolver first uses the official Overture Maps client against a pinned open Buildings release. If that source has no usable coverage, it falls back in order to the pinned Microsoft GlobalML Building Footprints release, an explicitly authorized Lee County building layer, and a cached OpenStreetMap Overpass response. Overture, Microsoft and OSM are one correlated open-map lineage rather than three votes. On the onsite/EagleView-verified Lee County calibration roofs, authoritative county polygons and the open-map roof outline may differ because one follows the structure and the other follows visible roof edges; calibrated consensus requires IoU at least 0.70, centroid separation no more than 4 m and area difference no more than 16%. A passing independent comparison corroborates the first successful cascade outline without replacing it, because a county structure polygon can include screened enclosures, patios, or other non-roof additions. Material or correlated-source disagreement still fails closed, and LiDAR, Open3D, Solar, current-evidence and confidence gates remain mandatory.
...
5. Open3D independently refits every Roofer facet from the complete high-precision, normalized roof-return coordinates, uses nearest-3D-plane assignment where LoD2.2 faces overlap in plan view, and rejects inadequate support, low inlier share, normal disagreement, or excess plane RMSE before any measurement can price. The earlier PDAL HAG, cluster, surface-normal and curvature gates are not repeated during export because a second k-neighbour pass can erase valid returns beside a hip or valley; Open3D receives the accepted evidence and applies its own RANSAC gates.
6. `cityjson_geometry.py` reads Roofer's semantic `RoofSurface` faces and derives 3D facet area, horizontal area, pitch degrees, rise-per-12, azimuth, eaves, rakes, valleys, ridges, hips, external perimeter, internal roof-line length, and flat-roof area from their topology. Every facet's mesh area must reconcile with `horizontal_area / cos(pitch_angle)`.
