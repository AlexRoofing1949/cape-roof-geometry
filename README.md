...
5. 3DBAG Roofer v1.0.0 reconstructs LoD2.2 roof planes from the classified point cloud and the selected roofprint. Cape Roof sets Roofer's plane-detection inlier distance to 0.15 m instead of the less precise 0.30 m upstream default, matching the independent Open3D RANSAC tolerance used for production evidence. The reconstruction uses Roofer's maximum supported complexity factor (`1.0`) during reference calibration so valid facets and roof-line intersections are not prematurely simplified.
