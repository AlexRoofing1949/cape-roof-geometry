...
5. The pinned 3DBAG Roofer v1.1.0-beta.1 candidate reconstructs LoD2.2 roof planes from the classified point cloud and the selected roofprint. It contains upstream repairs for non-manifold/self-intersecting junctions and deterministic building-level decisions that are required for Cape Roof's reference calibration; it remains in zero-traffic evaluation until repeatable EagleView comparisons pass. Cape Roof sets Roofer's plane-detection inlier distance to 0.15 m instead of the less precise 0.30 m upstream default, matching the independent Open3D RANSAC tolerance used for production evidence. Reference calibration uses complexity factor `0.95`; factor `1.0` retained sub-quarter-square-metre artifacts and therefore failed the existing minimum-facet safety gate.
...
The candidate Docker image pins the official `3dgi/roofer:v1.1.0-beta.1`, Micromamba, and Caddy images by immutable registry digest and installs PDAL 2.9.2 plus `overturemaps-py` 1.0.1. Roofer's GDAL/PROJ data paths are isolated to the Roofer subprocess; PDAL and `ogr2ogr` use the pinned conda data files. `/healthz` remains `503 not_ready` if required source provenance or executables are missing.
