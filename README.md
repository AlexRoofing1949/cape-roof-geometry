...
7. `cityjson_geometry.py` reads Roofer's semantic `RoofSurface` faces and derives 3D facet area, horizontal area, pitch degrees, rise-per-12, azimuth, eaves, rakes, valleys, ridges, hips, external perimeter, internal roof-line length, and flat-roof area from their topology. Before classification it clusters coincident 3D coordinates and nodes partial shared edges, because valid Roofer facets can meet geometrically without reusing the same CityJSON vertex IDs. Every facet's mesh area must reconcile with `horizontal_area / cos(pitch_angle)`. County-record sketches and plans may corroborate this reconstruction but cannot supply internal edge topology or authorize pricing by themselves.
