...
2. When enabled with a Secret Manager-backed Solar credential, the service obtains the authorized high-quality Google Solar 0.1 m building-mask and DSM GeoTIFFs and polygonizes the selected rooftop. It removes raster stair steps with a topology-preserving 0.15 m simplification and fails closed if that changes ground area by more than 2%. The resulting roof mask must match Solar's independent facet ground-area total within the production-calibrated 8% threshold and remain spatially consistent with the corroborated building footprint. The dated DSM is sampled across every independently recovered facet; each facet must meet the configured centered shape RMSE after removal of its audited vertical offset, and the full set of offsets must remain within the configured cross-facet range. This separates pitch/shape evidence from LiDAR height-above-ground datum variation without hiding inconsistent roof elevations. These transient rasters drive the requested reconstruction and are not redistributed. The API key and authenticated raster URLs are never returned or logged.
...
6. Before any edge classification, Open3D estimates local normals on the roofprint-cropped, high-precision returns and builds deterministic spatial components using reciprocal point-to-plane residuals. Globally non-planar connected chains are split with successively tighter evidence; disconnected sections remain separate. Adjacent fragments merge only when angle, elevation, hull distance, and a refit of the complete proposed group all agree. Google Solar pitch/azimuth facets and the dated DSM reject material conflicting support. Supported plane intersections split Roofer coverage, coplanar fragments merge, and the corrected CityJSON roof surfaces—not the original Roofer surface list—feed the canonical topology graph. Any incomplete, overlapping, unsupported, or DSM-conflicting partition fails closed. Open3D then independently refits every corrected facet and retains the existing inlier, normal-agreement, RMSE, and support-density gates.
7. `cityjson_geometry.py` reads Roofer's semantic `RoofSurface` faces and derives 3D facet area, horizontal area, pitch degrees, rise-per-12, azimuth, eaves, rakes, valleys, ridges, hips, external perimeter, internal roof-line length, and flat-roof area from their topology. Facet order, ring orientation, and starting vertices are coordinate-canonical so Roofer array order cannot change stable facet identities. Before classification the service clusters coordinates and nodes partial shared edges with a bounded 0.10 m tolerance, because valid Roofer facets can meet geometrically without reusing the same CityJSON vertex IDs. A proposed shared segment whose two reconstructed directions differ by more than 5 degrees is suppressed and audited as a crossing node artifact; it is never converted into a priced exterior edge. Non-manifold fragments no longer than the configured 0.15 m node tolerance are likewise suppressed and audited as noding artifacts, while every longer non-manifold edge still fails closed. Single-use eave/rake candidates must lie within 0.50 m of the accepted Google Solar roofprint boundary; horizontal low-side boundaries are eaves, and the remaining slope-direction test uses a local 0.75 m roofprint tangent so 0.1 m mask stair steps cannot turn eaves into rakes. Unmatched internal boundaries are retained in the audit, while high-side flashing is measured separately and excluded from exterior-perimeter reconciliation. Every accepted noncoplanar shared boundary is then independently snapped to the two incident facet planes' theoretical 3D intersection, using the Roofer boundary only for supported clipping limits and rejecting corrections over 0.35 m. The unchanged roofprint-perimeter and private reference-report gates must prove that suppressed artifacts did not remove required topology. The response records each derivation and rejection; material non-manifold or ambiguous topology still fails closed. Every facet's mesh area must reconcile with `horizontal_area / cos(pitch_angle)`. County-record sketches and plans may corroborate this reconstruction but cannot supply internal edge topology or authorize pricing by themselves. Official county wall footprints are compared with roof-overhang masks using both their raw overlap and their overlap after a bounded centroid alignment. Raw IoU of at least 0.65, centroid shift no greater than 4 m, centroid-aligned IoU of at least 0.70, and area change no greater than 16% are all required; this matches the independently corroborated provider-consensus threshold while tolerating documented orthophoto registration and overhang differences. Same-area material shape changes still fail closed.
...
python tools/eagleview_calibration.py /private/eagleview --output /private/eagleview/calibration-manifest.json --max-area-error-percent 1 --max-pitch-error-degrees 1 --max-edge-error-feet 1 --minimum-repeat-runs 2
```
...
Cape Roof candidate runs are supplied as two or more files named
`<report-id>_CapeRoofGeometry*.json` (or one such file containing a `runs` array). A report is not a calibration
pass unless every repeated run keeps roof area within 1%, pitch within 1 degree, exact facet count, the configured
ridge/hip/valley/rake/eave tolerance, a maximum cosine-formula error of `0.000001%`, and one identical canonical
topology hash. Missing candidate runs or any failed geometry remains `inspectionRequired:true`. The service also
keeps `pricingAllowed:false` while `FACET_CALIBRATION_APPROVED=false`; this tool never enables customer pricing.
